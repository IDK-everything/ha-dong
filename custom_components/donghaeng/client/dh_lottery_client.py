...
class DhLotteryClient:
    ...
    async def async_get_balance(self) -> DhLotteryBalanceData:
        ...
        try:
            ...
            _LOGGER.warning(f"[예치금 API 응답 원본] type={type(user_result)}, keys={list(user_result.keys()) if isinstance(user_result, dict) else 'N/A'}, value={str(user_result)[:500]}")
            ...
            _LOGGER.warning(f"[userMndp 파싱 결과] type={type(user_mndp)}, keys={list(user_mndp.keys()) if isinstance(user_mndp, dict) else 'N/A'}, value={str(user_mndp)[:300]}")
